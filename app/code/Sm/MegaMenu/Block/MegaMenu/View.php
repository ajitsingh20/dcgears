...
class View extends Template
{
	...
	public function getLinkOfType($item){
		...
		elseif($item['type'] == self::CMSPAGE){
			...
			if($storeManager->getStore()->getCode()=='us'){
			return $storeManager->getStore()->getBaseUrl(\Magento\Framework\UrlInterface::URL_TYPE_WEB).$item['data_type'];
			}else{
            		return $storeManager->getStore()->getBaseUrl(\Magento\Framework\UrlInterface::URL_TYPE_WEB).$storeManager->getStore()->getCode().'/'.$item['data_type'];
			}
        	}
		else
			return '#';
	...
	public function getCMSPageLink($item){
		...
		$url = $helperPage->getPageUrl($cmspageId);
		if($cmspageId==2){ $url = str_replace('/home','/', $url);}
		return $url;
	}
